...
setCentralWidget (centerFrame);  //新建的centerFrame->inside	Widget()对象作为主窗口的中央窗口
...
centerFrame->setPenWidth (widthSpinBox->value ());  //设置初	始线宽
...
{
QToolBar *toolBar = addToolBar (tr("Tool"));   //为主窗口创建	一个工具栏对象
...
//创建保存工具栏
savepicBtn = new QToolButton;
savepicBtn->setText (tr("保存"));
savepicBtn->setToolTip(tr("保存当前图片"));
connect (savepicBtn, &QToolButton::clicked, centerFrame, &CenterFrame::savepicPaint);
